...
contract("PairVoting", (accounts) => {
  beforeEach(async () => {
    this.initialManagers = accounts.slice(0, 4);
    ...
    for (const im of this.initialManagers) {
      await this.contract.addManager(im, {
        from: accounts[0],
      });
    }
    await this.contract.toProposalState({
      from: accounts[0],
    });
  });
...
contract("Tideman election", (accounts) => {
  beforeEach(async () => {
    this.initialManagers = accounts;
    this.owner = accounts[0];
    this.contract = await PairVoting.new(5, PROPOSAL_LIFETIME, VOTING_LIFETIME);
    for (const im of this.initialManagers) {
      await this.contract.addManager(im, {
        from: accounts[0],
      });
