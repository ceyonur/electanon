...
do
   ((p=p%N)); ((p++==0)) && wait
   NODE_OPTIONS=--max-old-space-size=4096 VROUND=$i ADDRESS=$ADDRESS VCOUNT=$VOTECOUNT PCOUNT=$PCOUNT BSIZE=$BATCHSIZE REPORT_GAS=true npx hardhat --network localhost test --no-compile ../gasanalysis/batched-basic/commit-vote.js > ../../outputs/electanon/basic/commit-vote-v$VOTECOUNT-p$PCOUNT-r$i.txt &
done
)
