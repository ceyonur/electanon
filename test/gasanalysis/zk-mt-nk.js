const BatchableMerkleForest = artifacts.require(
  "final/BatchableMerkleForestTestNK"
);
...
const verificationKeyFile = path.join(
  __dirname,
  "../../circuits/zk-mt-nk/build/verification.key"
);

const proofFile = path.join(
  __dirname,
  "../../circuits/zk-mt-nk/build/proof.json"
);
...
let identityBatch = [];
let proof;
contract("BatchableMerkleForestNK", (accounts) => {
  before(async () => {
  ...
  it("read valid proof for commitment batch", async () => {
    ...
    mtRoot = web3.utils.hexToNumberString(proof.inputs[treeSize]);
  ...
  it("inserts tree to contract", async () => {
    ...
    truffleAssert.eventEmitted(tx, "TreeInserted", (ev) => {
      return (
        expect(ev.treeIndex.toString()).to.be.equal("0") &&
  ...
  it("replace tree to contract", async () => {
    ...
    let innerProof = proof.proof;
    const tx = await batchableMerkleForest.replaceTreeTest(
      0,
      identityBatch,
      mtRoot,
      innerProof.a,
