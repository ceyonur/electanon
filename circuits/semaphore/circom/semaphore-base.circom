...
template CalculateNullifier(NULLIFIER_TRAPDOOR_SIZE_IN_BITS, EXTERNAL_NULLIFIER_SIZE_IN_BITS, n_levels) {
  ...
  // remove this, it might lead to double signalling as the same id can be registered twice to merkle tree
  // for (var i = 0; i < n_levels; i++) {
  //   nullifiers_hasher.in_bits[NULLIFIER_TRAPDOOR_SIZE_IN_BITS + EXTERNAL_NULLIFIER_SIZE_IN_BITS + i] <== identity_path_index[i];
  // }

  for (var i = (NULLIFIER_TRAPDOOR_SIZE_IN_BITS + EXTERNAL_NULLIFIER_SIZE_IN_BITS); i < nullifiers_hasher_bits; i++) {
    nullifiers_hasher.in_bits[i] <== n_levels;
  }
